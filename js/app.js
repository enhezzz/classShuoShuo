...
app.use(compress()); //gzip middleware
...
const accessOtherRouter = require("./modules/otherPH");
app.use("/accessOther", accessOtherRouter);
//-------------------------websocket------------------------------
...
app.post("/classShuoShuo/doRegist", function(req, res) {
  ...
  form.parse(req, function(err, fields, files) {
    ...
    db.insert("homePage", { cardId: fields.cardId, accessSum: 0 }, function() {
      console.log("初始化访问人数成功！");
    });
    res.redirect("login");
  });
});
...
app.get("/classShuoShuo/trends", function(req, res, next) {
  if (req.session.info) {
    if (req.query.r) {
      db.update(
        ...
        { $pull: { content: { serialCode: req.query.r } } },
        function(result) {
          console.log("删除说说成功!");
        }
      );
      ...
      db.find("shuoshuo", {}, function(rs) {
        ...
        rs.forEach((e, index) => {
          ...
          e.content.forEach(function(ele, i) {
            db.find(
              "comment",
              { cardId: e.cardId, serialCode: ele.serialCode },
              function(result) {
                if (result[0]) {
  ...
  } else res.redirect("/classShuoShuo");
  // next();
});
...
app.post("/classShuoShuo/trends/comment", function(req, res) {
  ...
  form.parse(req, function(err, fields, files) {
    ...
    db.find("comment", { cardId: cardId, serialCode: serialCode }, function(
      result
    ) {
      fields.date = new Date();
      ...
      if (result.length === 0) {
        ...
        db.insert(
          ...
          function(result) {
            console.log(
              req.session.info.name + "发表第一条" + cardId + "的评论成功"
            );
        ...
        db.update(
          "comment",
          { cardId: cardId, serialCode: serialCode },
          { $push: { comments: fields } },
          function() {
            let date = new Date();
            console.log(
              req.session.info.name + "发表" + cardId + "的评论成功!"
            );
            res.send(fields.date);
            res.end();
          }
...
app.get("/classShuoShuo/homePage", function(req, res, next) {
  if (req.session.info) {
    db.find("shuoshuo", { cardId: req.session.info.cardId }, function(result) {
      ...
      info.name = req.session.info.name;
      info.cardId = req.session.info.cardId;
      db.find("student", { cardId: req.session.info.cardId }, function(
        response
      ) {
        if (response.length) {
          ...
          info.date = date;
          info.className = response[0].class || "填写班级";
          info.site = response[0].site || "填写个人主网站";
        }
        db.find("homePage", { cardId: req.session.info.cardId }, function(
          response
        ) {
          if (response[0]) {
            info.accessSum = response[0].accessSum;
            console.log(info);
...
app.get("/classShuoShuo/homePage/:desc", function(req, res) {
  if (req.session.info) {
    if (req.params.desc === "querySS") {
      console.log("查询个人说说...");
      db.find("shuoshuo", { cardId: req.session.info.cardId }, function(
        result
      ) {
      ...
      db.find("shuoshuo", { cardId: req.session.info.cardId }, function(
        ...
        var info = {};
        info.shuoshuos = shuoshuos;
        res.render("homePage/shuoshuo.html", info);
      });
    } else if (req.params.desc === "charts") {
      db.find("article", { cardId: req.session.info.cardId }, function(
        response
      ) {
        if (response) {
          let allTags = [];
          ...
          response.forEach(function(ele) {
            allTags.push(...ele["tags"]);
          });
          allTags.forEach(function(ele) {
            if (data[ele] == undefined) data[ele] = 1;
            else data[ele] += 1;
          });
          for (let name in data) {
            dataSet.push({ value: data[name], name: name });
          }
          console.log(data);
          console.log(dataSet);
          console.log(allTags);
  ...
  } else res.redirect("/classShuoShuo/homePage");
});
app.get("/classShuoShuo/editClass", function(req, res) {
  if (req.session.info && req.query && req.query.class) {
    console.log(req.query.class);
    db.update(
      "student",
      { cardId: req.session.info.cardId },
      { $set: { class: req.query.class } },
      function() {
...
app.get("/classShuoShuo/editSite", function(req, res) {
  if (req.session.info && req.query && req.query.site) {
    console.log(req.query.site);
    db.update(
      "student",
      { cardId: req.session.info.cardId },
      { $set: { site: req.query.site } },
      function() {
...
app.get("/classShuoShuo/upload", function(req, res) {
  ...
  } else res.redirect("/classShuoShuo"); //重定向做会话判断
});
app.post("/classShuoShuo/doUpload", (req, res) => {
  ...
  if (req.session.info) {
    ...
    form.parse(req, function(err, fields, files) {
      ...
      fs.rename(
        files.file.path,
        path.resolve(__dirname, "../dir/" + fields.dir + "/" + files.file.name)
      );
...
app.post("/classShuoShuo/homePage/publishedArt", (req, res) => {
  ...
  if (req.session.info) {
    ...
    form.parse(req, function(err, fields, files) {
      ...
      Object.assign(fields, req.session.info, {
        date: new Date(),
...
app.get("/classShuoShuo/downLoad", (req, res) => {
  ...
  if (req.session.info) {
    if (req.query.dirName && !req.query.fileName) {
      ...
      fs.readdir("dir/" + dirName, function(err, files) {
        if (err) throw err;
        let fileSizes = [];
        files.forEach((file, index) => {
          let stat = fs.statSync("dir/" + dirName + "/" + file);
          let size = stat.size / 1024;
          if (size > 1024) size = (size / 1024).toFixed(2) + "M";
      ...
      res.download("dir/" + dirName + "/" + fileName);
    } else {
      fs.readdir("dir", function(err, files) {
        if (err) throw err;
        let obj = {};
...
app.post("/classShuoShuo/uploadTouXiang", (req, res) => {
  ...
  if (req.session.info) {
    ...
    form.parse(req, function(err, fields, files) {
      ...
      let touxiangDir = path.resolve(
        __dirname,
        "../touxiang/" + req.session.info.cardId
      );
      if (!fs.existsSync(touxiangDir)) {
        fs.mkdir(touxiangDir, function() {
          fs.rename(files.pic.path, touxiangDir + "/" + uuidv1() + extension);
        });
        res.end();
      } else {
