...
function cloneListForMapAndSample(original, excludeDimensions) {
  ...
  for (var i = 0; i < allDimensions.length; i++) {
    ...
    if (originalStorage[dim]) {
      storage[dim] = indexOf(excludeDimensions, dim) >= 0 ? cloneDimStore(originalStorage[dim]) // Direct reference for other dimensions
      : originalStorage[dim];
    }
  }
...
function adjustScaleForOverflow(min, max, model) {
  ...
  var barWidthAndOffset = calBarWidthAndOffset(filter(ecModel.getSeriesByType('bar'), function (seriesModel) {
...
function collectAxesInfo(result, ecModel, api) {
  ...
  each$7(api.getCoordinateSystems(), function (coordSys) {
    ...
    each$7(coordSys.getAxes(), curry$2(saveTooltipAxisInfo, false, null)); // If axis tooltip used, choose tooltip axis for each coordSys.
    ...
    && baseTooltipModel.get('show')) {
      ...
      if (triggerAxis || cross) {
        each$7(tooltipAxes.baseAxes, curry$2(saveTooltipAxisInfo, cross ? 'cross' : true, triggerAxis));
      }

      if (cross) {
        each$7(tooltipAxes.otherAxes, curry$2(saveTooltipAxisInfo, 'cross', false));
      }
...
function makeAxisPointerModel(axis, baseTooltipModel, globalAxisPointerModel, ecModel, fromTooltip, triggerTooltip) {
  ...
  var volatileOption = {};
  each$7(['type', 'snap', 'lineStyle', 'shadowStyle', 'label', 'animation', 'animationDurationUpdate', 'animationEasingUpdate', 'z'], function (field) {
...
function dispatchHighDownActually(axesInfo, dispatchAction, api) {
  ...
  var newHighlights = inner$6(zr)[highDownKey] = {}; // Update highlight/downplay status according to axisPointer model.
...
BaseAxisPointer.prototype = {
  ...
  createPointerEl: function (group, elOption, axisModel, axisPointerModel) {
    ...
    if (pointerOption) {
      var pointerEl = inner$8(group).pointerEl = new graphic[pointerOption.type](clone$4(elOption.pointer));
      group.add(pointerEl);
    }
  },
  ...
  createLabelEl: function (group, elOption, axisModel, axisPointerModel) {
    if (elOption.label) {
      var labelEl = inner$8(group).labelEl = new Rect(clone$4(elOption.label));
      group.add(labelEl);
